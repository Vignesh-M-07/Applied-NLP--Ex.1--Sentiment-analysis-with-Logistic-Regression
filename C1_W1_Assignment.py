...
def test_logistic_regression(test_x, test_y, freqs, theta, predict_tweet=predict_tweet):
    """
    Input:
        test_x: list of tweets (length m)
        test_y: numpy array shape (m,1) with labels 0/1
        freqs: frequency dict
        theta: (3,1) weights
    Output:
        accuracy: fraction of tweets classified correctly (float)
    """
    y_hat = []
    for tweet in test_x:
        prob = predict_tweet(tweet, freqs, theta)
        y_hat.append(1.0 if prob > 0.5 else 0.0)

    y_hat = np.array(y_hat)
    accuracy = np.mean(y_hat == np.squeeze(test_y))
    return accuracy
